...
always@(*) begin 
    ...
    if (current_state == READ) begin
        if (is_first_col) begin
            ...
            // === 滑動視窗模式 (3 reads) ===
            // 只讀取新的「右欄 (Right Column)」
            // 此時 gray_addr_base 已經移到新的中心點了
            // 我們要讀它的：右上(-127), 右(+1), 右下(+129)
            // 為什麼是這些 offset? 因為對於新中心點來說，這三個點構成了它的右欄
            case(count)
...
always@(posedge clk or posedge reset) begin 
    if(reset) begin
        ...
        // 只有在不是第一欄的時候才需要 Shift
        // 當準備算下一顆 (current base + 1) 時：
        // 舊的中欄(1) -> 變成新的左欄(0)
        // 舊的右欄(2) -> 變成新的中欄(1)
        ...
        if (is_first_col) begin
            // === 填滿模式 (Fill All) ===
            // 依照你原本的 count 對應位置填入 window_data
            case(count)
