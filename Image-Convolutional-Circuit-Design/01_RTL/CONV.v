...
reg signed[19:0] idata_buffer; // 用於暫存從記憶體讀取的資料，確保在乘法階段資料穩定
...
always @(*) begin
    case(count_kernel)
        ...
        default: kernel_w = 20'shFAC19; // k=8
    endcase
end
...
wire signed [7:0] offset_x = (count_kernel==4'd0 || count_kernel==4'd3 || count_kernel==4'd6) ? -8'sd1 :
                       (count_kernel==4'd1 || count_kernel==4'd4 || count_kernel==4'd7) ? 8'sd0 : 8'sd1;
...
always @(posedge clk or posedge reset) begin
    if (reset) begin
        ...
        if (!is_padding) 
            mult_reg <= idata_buffer * kernel_w;
        else 
            mult_reg <= 0;
    end
end
